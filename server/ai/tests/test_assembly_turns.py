...
class AssemblyTurnsTest(unittest.TestCase):
    def test_partial_then_final_and_formatted_duplicate(self):
        ...
        self.assertEqual(turns.receive({**base, "end_of_turn": False}), ("Bom dia turma", False, None))
        self.assertEqual(turns.receive({**base, "end_of_turn": True}), ("Bom dia turma", True, None))
        self.assertIsNone(turns.receive({**base, "end_of_turn": True, "turn_is_formatted": True}))
        self.assertEqual(turns.receive({**base, "turn_order": 1, "end_of_turn": True}), ("Bom dia turma", True, None))

    def test_v3_field_takes_precedence_and_empty_status_is_ignored(self):
        turns = AssemblyTurns()
        self.assertEqual(turns.receive({"type": "Turn", "transcript": "Aula", "end_of_turn": False, "turn_is_done": True}), ("Aula", False, None))
