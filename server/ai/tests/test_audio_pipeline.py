    ...
    async def send(self, data):
        ...
        if isinstance(data, bytes):
            for final in (False, True):
                self.incoming.put_nowait(json.dumps({'type': 'Turn', 'turn_order': 0, 'transcript': 'Bom dia turma', 'end_of_turn': final}))
...
class AudioPipelineTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        settings = replace(session.SETTINGS, assemblyai_api_key='mock-only', save_transcripts=False, sample_rate=16000, channels=1, chunk_size=1600)
