...
test('attachments: classroom ownership, membership, uploads and private downloads', async (t) => {
  ...
  global.fetch = (url, init) => String(url).startsWith(env.aiBackendUrl + '/materials/ingest')
    ? Promise.resolve({ ok: false }) : originalFetch(url, init);
  t.after(async () => {
  ...
  const room = { id: '6b484c0b-b9c1-4ab4-bd91-ea0f83a37463', teacherId: 'teacher', members: [{ userId: 'student' }, { userId: 'other-teacher' }], name: 'Sala', teacher: { name: 'Docente', teacherProfile: null }, code: 'ABC', lessons: [] };
  room.members = room.members.map(member => ({ ...member, user: users.find(user => user.id === member.userId), joinedAt: new Date() }));
  const lesson = { id: '80468ca5-f321-4ed4-93dc-66f8753de2b3', classroomId: room.id, classroom: room };
  const records = [];
  const hydrate = record => ({ ...record, classroom: record.classroomId === room.id ? room : null, lesson: record.lessonId === lesson.id ? lesson : null });
  ...
  prisma.material.create = async ({ data }) => {
    const record = { id: `material-${records.length}`, createdAt: new Date(), lessonId: null, classroomId: null, ...data };
    records.push(record);
    return record;
  };
  prisma.material.findMany = async ({ where }) => records.map(hydrate).filter(record => matches(record, where));
  prisma.material.findFirst = async ({ where }) => records.map(hydrate).find(record => matches(record, where)) ?? null;
  const request = (userId, route, method = 'GET', body) => originalFetch(base + route, {
    method,
    headers: { 'Content-Type': 'application/json', ...(userId ? { Authorization: `Bearer ${signToken({ sub: userId, email: 'test@example.com' })}` } : {}) },
    body: body === undefined ? undefined : JSON.stringify(body),
  });
  const attachment = { filename: 'atividade.txt', contentBase64: Buffer.from('Atividade da turma').toString('base64'), classroomId: room.id };
  ...
  for (const user of ['student', 'outsider', 'other-teacher']) {
    ...
    assert.equal((await request(user, '/materials', 'POST', { ...attachment, lessonId: lesson.id })).status, 403, user);
  }
  ...
  for (const patch of [
    ...
    assert.equal((await request('teacher', '/materials', 'POST', { ...attachment, ...patch })).status, 400, JSON.stringify(patch));
  }
  assert.equal(records.length, 0);
  ...
  const lessonResponse = await request('teacher', '/materials', 'POST', { ...attachment, classroomId: undefined, lessonId: lesson.id, contentBase64: 'data:text/plain;base64,' + attachment.contentBase64 });
  ...
  assert.equal((await request('student', `/education/materials/${records[0].id}/download`)).status, 404);
  room.members = [{ userId: 'student' }];
  // Legacy lesson attachments inherit the same room audience.
  records[1].classroomId = null;
  assert.equal((await request('student', `/education/materials/${records[1].id}/download`)).status, 200);
  ...
  assert.equal((await request('teacher', `/education/materials/${records[0].id}/download`)).status, 404);
  ...
  prisma.material.create = async () => { throw new Error('Simulated database failure'); };
  const { materialService } = require('../dist/services/material.service');
  await assert.rejects(() => materialService.upload('teacher', 'PROFESSOR', attachment), /Simulated database failure/);
