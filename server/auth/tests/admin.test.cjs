...
const { adminUserSchema, adminSchoolSchema, adminClassroomSchema, adminQuerySchema } = require('../dist/schemas/Admin.schema');
...
test('admin routes enforce role, account state, session version and safe serialization', async t => {
  const users = [
    { id: ADMIN, name: 'Admin', email: 'admin@example.com', role: 'ADMIN', isActive: true, sessionVersion: 0, passwordHash: 'private-hash' },
    { id: OTHER, name: 'Professor', email: 'teacher@example.com', role: 'PROFESSOR', isActive: true, sessionVersion: 0, passwordHash: 'private-hash' },
  ];
  ...
  for (const name of ['school', 'classroom', 'lesson', 'material']) prisma[name].count = async () => 0;
  let listArgs;
  prisma.user.findMany = async args => {
    listArgs = args;
    return users.map(user => Object.fromEntries(Object.keys(args.select).filter(key => key in user).map(key => [key, user[key]])));
  };
  ...
  const request = (id, route, method = 'GET', body) => fetch(base + route, { method, headers: { 'Content-Type': 'application/json', ...(id ? { Authorization: `Bearer ${token(id)}` } : {}) }, body: body === undefined ? undefined : JSON.stringify(body) });
  assert.equal((await request(null, '/admin/overview')).status, 401);
  for (const role of ['PROFESSOR', 'ALUNO']) {
    users[1].role = role;
    for (const route of ['/admin/overview', '/admin/options', '/admin/users', '/admin/schools', '/admin/classrooms', '/admin/lessons', '/admin/materials']) assert.equal((await request(OTHER, route)).status, 403, route);
  ...
  assert.equal((await request(ADMIN, '/admin/schools', 'POST', { name: 'School', state: 'ZZ' })).status, 400);
  const response = await request(ADMIN, `/admin/users?page=2&q=ana&role=ALUNO&active=false&schoolId=${SCHOOL}`);
...
function fakeTransaction(tx) {
  prisma.$transaction = async (action, options) => { assert.equal(options.isolationLevel, 'Serializable'); return action(tx); };
}
...
test('self protection, last admin, blocking and immediate session revocation', async () => {
  ...
  await assert.rejects(adminService.setUserActive(ADMIN, ADMIN, false), error => error.statusCode === 400);
  await assert.rejects(adminService.deleteUser(ADMIN, ADMIN), error => error.statusCode === 400);
  await assert.rejects(adminService.setUserActive(ADMIN, OTHER, false), error => error.statusCode === 409);
  count = 2;
  ...
  assert.equal(await comparePassword('Uma senha administrativa 2026!', updated.data.passwordHash), true);
  assert.deepEqual(updated.data.sessionVersion, { increment: 1 });
});

test('role changes require classroom transfer and school references must exist', async () => {
  fakeTransaction({
    user: { findUnique: async ({ where }) => where.id === ADMIN ? actor : { id: OTHER, role: 'PROFESSOR', isActive: true } },
    classroom: { count: async () => 1 },
  });
  const input = adminUserSchema.parse({ name: 'Novo aluno', email: 'novo@example.com', role: 'ALUNO', registrationNumber: '001', schoolId: null });
  await assert.rejects(adminService.saveUser(ADMIN, OTHER, input), error => error.statusCode === 409);
  fakeTransaction({ user: { findUnique: async () => actor }, school: { findUnique: async () => null } });
  await assert.rejects(adminService.saveClassroom(ADMIN, undefined, { name: 'Sala', description: '', teacherId: OTHER, schoolId: SCHOOL }), error => error.statusCode === 400);
});
...
test('user create/update normalizes data, hashes passwords, keeps profiles and roles consistent', async () => {
  ...
  fakeTransaction({
    user: {
      findUnique: async ({ where }) => where.id === ADMIN ? actor : { id: OTHER, role: 'ALUNO', email: 'old@example.com', isActive: true },
      findFirst: async () => null,
      create: async ({ data }) => { stored = data; return { id: OTHER }; },
      update: async ({ data }) => { stored = data; return { id: OTHER }; },
      findUniqueOrThrow: async () => ({ id: OTHER, name: stored.name, email: stored.email, role: stored.role }),
    },
    school: { findUnique: async () => ({ id: SCHOOL, name: 'Escola Central' }) },
    teacherProfile: { upsert: async args => calls.push(args), deleteMany: async () => {} },
    studentProfile: { upsert: async args => calls.push(args), deleteMany: async () => calls.push('student-deleted') },
  });
  const data = adminUserSchema.parse({ name: '  Ana   Maria ', email: ' ANA@EXAMPLE.COM ', role: 'PROFESSOR', discipline: 'Matemática', schoolId: SCHOOL, password: 'Uma senha administrativa 2026!' });
...
test('room ownership transfers its lessons and requires an active teacher', async () => {
  ...
  let lessons;
  fakeTransaction({
    user: { findUnique: async () => actor, findFirst: async ({ where }) => { assert.equal(where.role, 'PROFESSOR'); assert.equal(where.isActive, true); return activeTeacher ? { id: OTHER } : null; } },
    school: { findUnique: async () => ({ id: SCHOOL }) },
    classroom: { update: async args => { classroom = args; return { id: ROOM }; } },
    lesson: { updateMany: async args => { lessons = args; return { count: 1 }; } },
  });
  const data = { name: 'Sala transferida', description: '', schoolId: SCHOOL, teacherId: OTHER };
  await assert.rejects(adminService.saveClassroom(ADMIN, ROOM, data), error => error.statusCode === 400);
  activeTeacher = true;
...
test('deletions preserve linked content and reject stale admin privileges', async () => {
  let deletes = 0;
  fakeTransaction({
    user: { findUnique: async ({ where }) => where.id === ADMIN ? actor : { id: OTHER, role: 'ALUNO', isActive: true }, count: async () => 1, findUniqueOrThrow: async () => ({ _count: { materials: 1 } }), delete: async () => deletes++ },
    classroom: { count: async () => 0, findUniqueOrThrow: async () => ({ _count: { lessons: 2 } }), delete: async () => deletes++ },
    material: { count: async () => 1 },
    school: { delete: async () => deletes++ },
    lesson: { delete: async () => deletes++ },
  });
  await assert.rejects(adminService.deleteSchool(ADMIN, SCHOOL), error => error.statusCode === 409);
  await assert.rejects(adminService.deleteUser(ADMIN, OTHER), error => error.statusCode === 409);
  await assert.rejects(adminService.deleteClassroom(ADMIN, ROOM), error => error.statusCode === 409);
  ...
  await assert.rejects(adminService.deleteMaterial(ADMIN, ROOM), error => error.statusCode === 403);
});

test('admin input validation rejects privilege injection, retired roles and invalid data', () => {
  const valid = { name: 'Usuário', email: 'user@example.com', role: 'ALUNO', registrationNumber: '001' };
  for (const patch of [{ role: 'SOCIEDADE' }, { password: 'weak' }, { isActive: true }, { passwordHash: 'injected' }, { sessionVersion: 0 }, { registrationNumber: '  ' }, { schoolId: 'invalid' }]) assert.equal(adminUserSchema.safeParse({ ...valid, ...patch }).success, false);
...
test('new lessons use the room teacher and existing lessons cannot change rooms', async () => {
  let created;
  fakeTransaction({
    user: { findUnique: async () => actor, findFirst: async () => ({ id: OTHER, role: 'PROFESSOR', isActive: true }) },
    classroom: { findUniqueOrThrow: async () => ({ id: ROOM, teacherId: OTHER }) },
    lesson: { create: async args => { created = args.data; return { id: SCHOOL }; } },
  });
  await assert.rejects(adminService.saveLesson(ADMIN, undefined, { title: 'Aula', status: 'AGENDADA' }), error => error.statusCode === 400);
  await adminService.saveLesson(ADMIN, undefined, { title: 'Aula nova', status: 'AGENDADA', classroomId: ROOM });
  ...
  await assert.rejects(adminService.saveLesson(ADMIN, SCHOOL, { title: 'Aula', status: 'AGENDADA', classroomId: ROOM }), error => error.statusCode === 400);
});
