...
const valid = { name: "  Ana   D’Ávila  ", email: ' ANA@EXAMPLE.COM ', password: 'Uma frase exclusiva 2026!', role: 'ALUNO', registrationNumber: ' 001-A ' };
...
test('rejects invalid names, emails, types and public admin registration', () => {
  for (const name of ['  ', '1', '12345', '<script>alert(1)</script>', 'Ana\u200bMaria', 'Ana\nMaria', 'A'.repeat(151)]) {
    assert.equal(registerSchema.safeParse({ ...valid, name }).success, false, name);
  }
  for (const email of ['x', 'a@localhost', 'a..b@example.com', 'a.@example.com', 'a b@example.com', 'a@-example.com', `${'a'.repeat(65)}@example.com`]) {
    assert.equal(registerSchema.safeParse({ ...valid, email }).success, false, email);
  }
  for (const patch of [{ name: null }, { email: 123 }, { password: [] }, { role: 'ADMIN' }, { role: 'other' }]) {
    assert.equal(registerSchema.safeParse({ ...valid, ...patch }).success, false);
  }
});

test('requires profile fields and bounds optional text', () => {
  for (const patch of [{ registrationNumber: '  ' }, { role: 'PROFESSOR', discipline: ' ' }, { institution: 'a'.repeat(151) }, { discipline: 'a'.repeat(101) }, { registrationNumber: 'a'.repeat(51) }, { institution: 'School\nName' }]) {
    assert.equal(registerSchema.safeParse({ ...valid, ...patch }).success, false);
  }
  assert.equal(registerSchema.safeParse({ ...valid, role: 'PROFESSOR', discipline: 'Matemática', registrationNumber: undefined }).success, true);
});

test('password policy respects UTF-8 bcrypt boundaries and legacy login', () => {
  for (const password of ['12345678', ' '.repeat(15), 'a'.repeat(20), 'Abcdefghijklmno\0', 'é'.repeat(35) + 'abc', 'abc'.repeat(25)]) {
  ...
  assert.equal(registerSchema.safeParse({ ...valid, password: 'é'.repeat(34) + 'abcd' }).success, true);
  assert.equal(loginSchema.parse({ email: valid.email, password: 'oldpass' }).email, 'ana@example.com');
  assert.equal(loginSchema.safeParse({ email: valid.email, password: '' }).success, false);
  assert.equal(loginSchema.safeParse({ email: valid.email, password: 'x'.repeat(1025) }).success, false);
});
...
test('browser and API validation rules remain identical', { skip: !fs.existsSync(browserRules) }, () => {
  assert.equal(fs.readFileSync(browserRules, 'utf8'), fs.readFileSync(path.resolve(__dirname, '../src/validation/account.ts'), 'utf8'));
});
