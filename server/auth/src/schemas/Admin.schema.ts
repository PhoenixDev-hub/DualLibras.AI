import { z } from 'zod';
import { accountEmailSchema, accountNameSchema, accountPasswordSchema, profileTextSchema } from './Account.schema';
import { RoleSchema, LessonStatusSchema } from './Enums.schema';

const text = (max: number) => z.string().trim().max(max).refine(value => !/[\p{Cc}\p{Cf}<>]/u.test(value), 'Texto contém caracteres inválidos');
...
export const adminSchoolSchema = z.object({
  ...
  state: z.union([z.literal(''), z.enum(['AC','AL','AP','AM','BA','CE','DF','ES','GO','MA','MT','MS','MG','PA','PB','PR','PE','PI','RJ','RN','RS','RO','RR','SC','SP','SE','TO'])]).default(''),
}).strict();
export const adminUserSchema = z.object({
  ...
  if (data.role === 'PROFESSOR' && !data.discipline) ctx.addIssue({ code: 'custom', path: ['discipline'], message: 'Disciplina é obrigatória para professores' });
  if (data.role === 'ALUNO' && !data.registrationNumber) ctx.addIssue({ code: 'custom', path: ['registrationNumber'], message: 'Matrícula é obrigatória para alunos' });
});
...
export const adminClassroomSchema = z.object({
