...
export const PASSWORD_HINT =
  'Use pelo menos 15 caracteres. Prefira uma frase longa e exclusiva. Limite de 72 bytes (acentos e emojis ocupam mais de um byte).'
...
export function nameError(value: string): string | undefined {
  ...
  if (unsafeText.test(value) || !/\p{L}/u.test(name))
    return 'Informe um nome válido, sem caracteres de controle ou marcação'
}
...
export function passwordError(value: string, login = false): string | undefined {
  ...
  if (new Set(value.replace(/\s/g, '').toLowerCase()).size < 4)
    return 'Evite senhas repetitivas ou compostas apenas por espaços'
}
