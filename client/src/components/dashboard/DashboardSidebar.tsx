...
export default function DashboardSidebar({ activeSection, menuItems, user, onSectionChange }: DashboardSidebarProps) {
      ...
      <section className="border-t border-slate-200 pt-5 dark:border-slate-800">
        <button className="flex w-full items-center gap-3 rounded-lg p-2 text-left transition hover:bg-slate-100 dark:hover:bg-slate-800">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="block truncate text-xs text-slate-500 dark:text-slate-400">{profileLabel}</span>
          </span>
        </button>
        <button onClick={handleLogout} className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-slate-200 text-sm font-bold text-slate-700 transition hover:border-red-200 hover:bg-red-50 hover:text-red-700 dark:border-slate-800 dark:text-slate-300 dark:hover:border-red-900 dark:hover:bg-red-950/40 dark:hover:text-red-300">
