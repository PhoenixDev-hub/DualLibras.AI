...
export default function VLibrasStage({
  ...
  return (
    <div
      ...
      className={`lesson-avatar relative flex flex-col justify-between overflow-hidden rounded-3xl border border-sky-500/25 bg-gradient-to-b from-slate-900/90 via-[#020b24]/90 to-black/95 p-4 shadow-[0_20px_60px_rgba(2,11,43,0.8),inset_0_1px_1px_rgba(130,227,255,0.2)] backdrop-blur-xl transition-all duration-300 ${className}`}
    >
      ...
      {!expanded && (
        <header className="relative z-30 flex items-center justify-between gap-2 flex-wrap border-b border-white/10 pb-3">
          <div className="flex items-center gap-2">
            <div className="grid h-8 w-8 place-items-center rounded-lg border border-sky-400/40 bg-sky-500/15 text-sky-300">
            ...
            <div>
              ...
              <p className="text-[11px] font-medium text-slate-400">Libras 3D • Ícaro</p>
            </div>
          </div>
          ...
          <div
            className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold transition-all ${
              isTranslating
                ? 'border border-sky-400/50 bg-sky-500/25 text-sky-200 shadow-[0_0_12px_rgba(56,189,248,0.5)]'
                : status === 'error'
      ...
      >
        ...
        {status === 'error' && (
          <div className="flex flex-col items-center gap-2 text-center text-red-300 p-4">
        ...
        {!expanded && isTranslating && activeWord && (
          <div className="absolute bottom-6 left-1/2 z-30 -translate-x-1/2 whitespace-nowrap rounded-xl border border-sky-400/40 bg-slate-950/85 px-4 py-1.5 text-xs font-black text-white shadow-[0_4px_20px_rgba(0,0,0,0.8),0_0_15px_rgba(56,189,248,0.4)] backdrop-blur-md transition-all">
            <span className="mr-1.5 text-sky-400">Leitura:</span>
            <span className="text-sky-200 uppercase tracking-wide">{activeWord}</span>
          </div>
        )}
      </div>

      <footer className="avatar-stage-footer relative z-30 flex items-center justify-between flex-wrap gap-3 border-t border-white/10 pt-3 text-xs">
        {expanded && (
        ...
        {onReload && (
          <button
            type="button"
            onClick={onReload}
            className="flex cursor-pointer items-center gap-1 rounded-lg border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] font-bold text-slate-300 transition-all hover:bg-white/10 hover:text-white"
