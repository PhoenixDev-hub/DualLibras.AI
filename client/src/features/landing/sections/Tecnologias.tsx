      ...
      <div className="max-w-5xl mx-auto text-center">
        ...
        >
          ...
          <p className="mt-6 max-w-2xl mx-auto text-base md:text-lg text-gray-mid leading-relaxed font-text">
            Uma stack pensada para latência baixa e tradução confiável, do captador de áudio no
            navegador até o processamento de voz e a geração de Libras em tempo real.
          </p>
        </div>
            ...
            >
              ...
              <ul className="space-y-3">
                {items.map(({ icon: Icon, label }) => (
                  <li
                    key={label}
                    className="flex items-center gap-3 text-sm text-gray-mid font-text"
                  >
                    <Icon
                      size={16}
                      strokeWidth={1.75}
            ...
            {archNodes.map(({ icon: Icon, title, subtitle }, index) => (
              <div key={title} className="flex items-center gap-6 md:gap-4">
                ...
                >
                  ...
                  <span className="mt-1 text-xs text-gray-mid font-text leading-snug">
                    {subtitle}
                  </span>
                </div>
